 ...
 */
  ...
  Returns - Int
   */
  def listSize(input:List[Int]):Int = input.size
  ...
  Returns - Int
   */
  def sumInts( input:Int ) : Int = input +25
...
*/
  def upper(input:List[String]):List[String] = input.map(xs => xs.toUpperCase)
  ...
  Params - List[String]
  Returns - List[String]
  input.exists(input => a.contains(input))
  input.exists(a.contains)
 */
  val a = "car"
